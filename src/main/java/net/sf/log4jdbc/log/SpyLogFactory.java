...
public class SpyLogFactory {
     ...
     * using the {@link net.sf.log4jdbc.Properties#getSpyLogDelegatorName()}.
     * If the name is <code>null</code>, load {@link net.sf.log4jdbc.log.slf4j.Slf4jSpyLogDelegator},
     ...
     * If <code>null</code>, load {@link net.sf.log4jdbc.log.slf4j.Slf4jSpyLogDelegator},
     * otherwise, try to load the corresponding class.
     *
     * @param spyLogDelegatorName A <code>String</code> representing the name of the class implementing
     *                            <code>SpyLogDelegator</code> to load.
     *                            If <code>null</code>, load <code>Log4j2SpyLogDelegator</code>.
    ...
    public static void loadSpyLogDelegator(String spyLogDelegatorName) {
        try {
            Object loadedClass = Class.forName(spyLogDelegatorName).getDeclaredConstructor().newInstance();
            setSpyLogDelegator((SpyLogDelegator) loadedClass);
        } catch (Exception e) {
            throw new IllegalArgumentException(
                    "spyLogDelegatorName does not allow to load a valid SpyLogDelegator: " + e.getMessage());
    ...
    /**
     * Set the <code>SpyLogDelegator</code> to use.
     *
