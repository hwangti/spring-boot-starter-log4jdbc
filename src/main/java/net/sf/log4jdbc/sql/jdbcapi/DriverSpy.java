 ...
 * code changes.
 *
 * <p>The well known driver classes that are loaded are:</p>
 * <ul>
 ...
 * </ul>
 *
 * <p>Additional drivers can be set via a property: <b>log4jdbc.drivers</b>
 * This can be either a single driver class name or a list of comma separated
 * driver class names.</p>
 ...
 *
 * <p>If any of the above driver classes cannot be loaded, the driver continues on
 * without failing.</p>
 *
 * <p>Note that the <code>getMajorVersion</code>, <code>getMinorVersion</code> and
 * <code>jdbcCompliant</code> method calls attempt to delegate to the last
 * underlying driver requested through any other call that accepts a JDBC URL.</p>
 *
 ...
 * its URL from the DriverManager in the first place (thus establishing an
 * underlying real driver), and in most applications there is only one database.
 *
 * @author Arthur Blake
...
public class DriverSpy implements Driver {
    ...
    static {
        ...
        // The Set of drivers that the log4jdbc driver will preload at instantiation time.
        // The driver can spy on any driver type, it's just a little bit easier to configure log4jdbc
        ...
        try {
            ...
            // this exception should never be thrown, JDBC just defines it for completeness
    ...
    public Connection connect(String url, java.util.Properties info) throws SQLException {
        ...
        // get actual URL that the real driver expects (strip off <code>#log4jdbcUrlPrefix</code> from url)
        url = this.getRealUrl(url);
     ...
     *
     * @param methodCall method call description
