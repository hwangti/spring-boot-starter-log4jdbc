...
public class DefaultResultSetCollector implements ResultSetCollector {
     ...
     * Default constructor.
     *
     * @param fillInUnreadValues if true, unread values will be filled in with the actual values
     */
    ...
    private static final List<String> GETTERS = Arrays.asList(
    ...
    public void loadMetaDataIfNeeded(ResultSet rs) {
        ...
        // otherwise, get all data now, so that we don't need to use the ResultSetMetaData later
        // (with some drivers, it cannot be used once the ResultSet has been closed)
        try {
            if (!rs.isClosed()) {
    ...
    private void loadMetaDataIfNeeded(ResultSetMetaData metaData)
    {
        ...
        try {
            ...
            this.colNameToColIndex = new HashMap<>(this.columnCount);
    ...
    /**
     * @param column An <code>int</code> representing the index of the column for which
    ...
    /**
     * Returns the column label using the column index number. The index starts from 1.
     *
     * @param column Returns the column label using the column index number. The index starts from 1.
     * @return the result set column label for a given column number previously obtained
     */
                                  ...
                                  Object targetObject, Object... methodParams)
    {
        ...
        if ("next()".equals(methodCall) || "first()".equals(methodCall) ||
            ...
            boolean isEndOfResultSet =
                    ...
                    ("close()".equals(methodCall) && this.lastValueReturnedByNext) ||
